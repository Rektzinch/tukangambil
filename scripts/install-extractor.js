...
async function install(){const response=await fetch(source,{redirect:"follow",headers:{"User-Agent":"TukangAmbil-Build/2.0"}});if(!response.ok||!response.body)throw new Error(`Gagal mengunduh yt-dlp (${response.status}).`);fs.mkdirSync(path.dirname(destination),{recursive:true});await pipeline(Readable.fromWeb(response.body),fs.createWriteStream(temp));const body=fs.readFileSync(temp);if(body.subarray(0,4).toString("hex")!=="7f454c46")throw new Error("Binary yt-dlp bukan ELF Linux.");const actual=crypto.createHash("sha256").update(body).digest("hex");if(expectedSha&&expectedSha!==actual)throw new Error("Checksum yt-dlp tidak cocok.");fs.renameSync(temp,destination);fs.chmodSync(destination,0o755);console.log(`yt-dlp siap sha256=${actual.slice(0,12)}…`)}install().catch(error=>{fs.rmSync(temp,{force:true});console.error(error.message);process.exitCode=1});
