...
const { hostnameMatches, safeFilename, mimeFromFilename, verifyDownloadToken } = require("../lib/core");
...
function requesterIp(req) { return String(req.headers?.["x-forwarded-for"] || req.socket?.remoteAddress || "unknown").split(",")[0].trim(); }
function rateLimit(req) { const now=Date.now(), ip=requesterIp(req), existing=buckets.get(ip); if(!existing || now-existing.started>60_000){buckets.set(ip,{started:now,count:1});return true} existing.count+=1; return existing.count<=40; }
...
module.exports=async function handler(req,res){
  ...
  if(!rateLimit(req))return res.status(429).json({error:"Terlalu banyak permintaan. Coba lagi sebentar."});
  ...
  try{
    ...
    if(preview){for(const h of ["accept-ranges","content-range"]){const v=upstream.headers.get(h);if(v)res.setHeader(h,v)}}
    let received=0;
  ...
  }catch(error){if(!res.headersSent)return res.status(502).json({error:"File belum dapat diunduh.",detail:String(error.message||error).slice(0,180)});res.destroy(error)}
};
