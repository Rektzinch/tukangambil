...
async function requestWavy(classified, mode) {
  ...
  const videos = Array.isArray(post?.media?.videos) ? post.media.videos.filter(item => item?.url) : [];
  const images = Array.isArray(post?.media?.images) ? post.media.images.filter(item => item?.url) : [];
  const source = classified.platform === "threads" && videos.length && !images.length ? videos.slice(0, 1) : media;
  let items = source.map(m => convertItem(m, post?.title, post?.cover)).filter(item => item?.url);
  if (mode === "image") items = items.filter(i => i.type === "image");
  else if (mode === "audio") items = items.filter(i => i.type === "audio");
  else if (mode === "mute") items = items.filter(i => i.type === "video" && i.hasAudio === false);
  ...
  return {
    platform: classified.platform || data?.platform || "unknown",
    provider: "wavy",
