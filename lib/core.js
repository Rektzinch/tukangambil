...
function classifyUrl(input) {
  ...
  if (platform === "tiktok") {
    ...
    else if (parts.some(part => /^(?:video|photo)$/i.test(part))) kind = "post";
    ...
    else if (parts.includes("post")) kind = "post";
  } else if (platform === "x") {
    handle = parts[0] || "";
...
function isProfileUrl(platform, value) {
  const classified = classifyUrl(value);
  return Boolean(classified && classified.platform === platform && classified.kind === "profile");
}
...
function safeName(value, fallback = "media") {
  ...
  return (withoutDangerousSuffix || fallback).slice(0, 72);
}

function safeFilename(value, fallback = "media") {
  return safeName(value, fallback).slice(0, 96);
}
...
function mediaTypeFromExtension(ext, fallback = null) {
  ...
  return Object.entries(EXTENSIONS).find(([, values]) => values.has(normalized))?.[0] || fallback;
}
...
function normalizeItem(item, index) {
  if (!item || !["image", "video", "audio"].includes(item.type) || !validateHttpsUrl(item.url)) return null;
  ...
  return {
    type: item.type,
    url: item.url,
    thumb: validateHttpsUrl(item.thumb) ? item.thumb : null,
    filename,
...
function validateResult(result, { mode = "auto" } = {}) {
  ...
  for (const [index, raw] of (Array.isArray(result.items) ? result.items : []).entries()) {
    ...
    if (mode === "mute" && item.type !== "video") continue;
  ...
  if (!items.length) throw Object.assign(new Error(mode === "audio" ? "Audio tidak tersedia pada tautan ini." : "Tidak ada media kompatibel pada tautan ini."), { code: "NO_COMPATIBLE_MEDIA" });
  return {
    ...
    warnings: Array.isArray(result.warnings) ? result.warnings.map(String).slice(0, 6) : [],
    items
  };
}
...
function verifyDownloadToken(token) {
  ...
  try {
    ...
    if (!data?.u || !data?.f || Number(data.e) < Math.floor(Date.now() / 1000)) return null;
...
module.exports = {
  ...
  hostnameMatches, platformOf, classifyUrl, isProfileUrl, canonicalProfileUrl,
  ...
  signDownloadToken, verifyDownloadToken, attachDownloadTokens, publicOrigin
};
