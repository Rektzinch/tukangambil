...
async function getUserId(username, session = null) {
  const endpoint = new URL(PROFILE_INFO_URL);
  endpoint.searchParams.set("username", username);
  const requestHeaders = session
    ? { ...headers(), "User-Agent": GRAPHQL_USER_AGENT, Cookie: session.cookies, "X-CSRFToken": session.csrf, "X-IG-WWW-Claim": "0", Referer: `https://www.instagram.com/${encodeURIComponent(username)}/` }
    : headers();
  const response = await fetch(endpoint, { headers: requestHeaders, signal: AbortSignal.timeout(PROVIDER_TIMEOUT_MS) });
  const payload = await response.json().catch(() => null);
  const user = payload?.data?.user;
  if (!response.ok || !user) throw Object.assign(new Error("Profil Instagram tidak dapat dibaca dari sumber publik."), { code: "INSTAGRAM_WEB_PROFILE_FAILED" });
  return user;
}

async function requestWebProfile(username, options = {}, session = null) {
  ...
  const user = await getUserId(username, session);
...
async function requestProfile(username, options = {}) {
  ...
  try {
    ...
    if (order === "newest") return requestWebProfile(username, { limit, offset, order }, session);
    throw error;
  }
}
