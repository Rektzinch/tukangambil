...
function allowedMediaUrl(value) { try { const u=new URL(value); if(u.protocol !== "https:" || u.username || u.password)return false; const host=u.hostname.toLowerCase(); if(host === "ssscdn.io")return !u.port && u.pathname.startsWith("/getmyfb/"); return hostnameMatches(host, [...MEDIA_HOSTS,...configuredHosts()]); } catch { return false; } }
function mediaRequestHeaders(url, range, preview){ const host=new URL(url).hostname.toLowerCase(); const h={Accept:"*/*","Accept-Encoding":"identity","User-Agent":"Mozilla/5.0 Chrome/127"}; if(range)h.Range=range; if(host.includes("tiktok"))h.Referer="https://www.tiktok.com/"; else if(host.includes("instagram"))h.Referer="https://www.instagram.com/"; else if(host.includes("fbcdn"))h.Referer="https://www.facebook.com/"; else if(host.includes("twimg"))h.Referer="https://x.com/"; else if(host.includes("ssscdn"))h.Referer="https://getmyfb.com/"; return h; }
